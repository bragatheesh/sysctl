...
register_command(char* buffer){
    ...
    sscanf(token, "%s", cmd_name);
    token = strtok(NULL, sep);
    sscanf(token, "%s", cmd_dir);

    HASH_FIND_STR(hash_commands, cmd_name, c);  //Check if command is already present in hash
    if (c != NULL) {
        syslog(LOG_NOTICE,"flexctl: Error: command %s has already been registered.", cmd_name);
        return -1;
    }
    ...
    syslog(LOG_NOTICE,"flexctl: %s %s", cmd_name, cmd_dir);
    return 0;
}
...
show(char* buffer){
    ...
    HASH_FIND_STR(hash_commands, cmd_name, c);  //Check if command is already present in hash
    if (c == NULL) {
        syslog(LOG_NOTICE,"flexctl: Error: command %s has not been registered.", cmd_name);
        return -1;
    }
